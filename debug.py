class LightPuzzle:
    def __init__(self):
        self.statearray = [[1, 1, 1], [1, 1, 0], [1, 0, 0]]
        self.solved = False
    ...
    def FlipTile(self, x, y):
        print(f"\nFlipping tile at ({x+1}, {y+1})")
        self.statearray[y][x] = self.ChangeState(self.statearray[y][x])

        if y > 0:  # flip the tile above
            self.statearray[y-1][x] = self.ChangeState(self.statearray[y-1][x])
        if y < len(self.statearray) - 1:  # flip the tile below
            self.statearray[y+1][x] = self.ChangeState(self.statearray[y+1][x])
        if x > 0:  # flip the tile to the left
            self.statearray[y][x-1] = self.ChangeState(self.statearray[y][x-1])
        if x < len(self.statearray[y]) - 1:  # flip the tile to the right
            self.statearray[y][x+1] = self.ChangeState(self.statearray[y][x+1])
